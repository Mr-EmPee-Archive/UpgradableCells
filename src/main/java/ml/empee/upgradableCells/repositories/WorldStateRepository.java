...
public class WorldStateRepository implements Bean {
  ...
  private void syncSave(WorldState data) {
    var query = "INSERT INTO worlds_state (world, last_cell, margin, size) VALUES (?, ?, ?, ?);";
    try (var stm = client.getJdbcConnection().prepareStatement(query)) {
      stm.setString(0, data.getWorld().getName());
      stm.setInt(1, data.getLastCell());
      stm.setInt(2, data.getMargin());
  ...
  private Optional<WorldState> syncFindByWorld(World world) {
    var query = "SELECT * FROM cells WHERE world = ?";
    try (var stm = client.getJdbcConnection().prepareStatement(query)) {
      stm.setString(0, world.getName());
